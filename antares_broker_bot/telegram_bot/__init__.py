...
class TelegramBot:
    ...
    def run(self, loop: asyncio.AbstractEventLoop, queues: Dict[str, AioQueue]):
        logging.info(f'Running telegram bot')
        self.loop = loop

        tasks = [loop.create_task(self.broadcast_alerts(topic=topic, queue=queue)) for topic, queue in queues.items()]
    ...
    async def broadcast_alerts(self, topic: str, queue: AioQueue) -> None:
        logging.info('Starting broadcast alerts')
        count = 0
        while True:
            try:
            ...
            logging.info(f'Just got locus from queue: {locus.locus_id}')
            ...
            for user_id in users:
                if await self.db.add_locus_if_not_exists(user_id=user_id, topic_name=topic, locus_id=locus.locus_id):
                    if await self.send_message(user_id, msg):
                        count += 1
                        if count % 100 == 0:
                            logging.info(f'Broadcast alerts: {count}')
    ...
    async def start_cmd(self, message: types.Message):
        ...
        msg = 'Hello, ANTARES alert bot is starting to send you alerts'
        logging.info(f'Answering: {msg}')
        await message.answer(msg)

    async def send_message(self, user_id: int, text: str, disable_notification: bool = True) -> bool:
        logging.info(f'Sending message to {user_id}: {text}')
        try:
